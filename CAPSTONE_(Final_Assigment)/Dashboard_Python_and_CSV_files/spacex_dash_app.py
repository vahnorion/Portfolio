...
# Create a list of labels with all Launch Sites
site_list = [{'label': i, 'value': i} for i in spacex_df['Launch Site'].unique()]
...
app.layout = html.Div(style={'width':'80%', 'margin':'0px auto'},
...
def get_scatter_plot(entered_site, selected_range):
    filtered_df = spacex_df[spacex_df['Payload Mass (kg)'].between(min(selected_range),max(selected_range))]
    if entered_site == 'ALL':
